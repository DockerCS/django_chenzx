...
class IndexView(View):
    ...
    def get(self, request):
        ...
        hot_favorite_sql = "SELECT `post_article`.`id`, `post_article`.`title` FROM post_article LEFT JOIN (SELECT * FROM tools_favorite WHERE favorite_type = 1) AS c ON post_article.id = c.favorite_id WHERE `post_article`.`is_published` = TRUE GROUP BY post_article.id ORDER BY count( c.favorite_id ) DESC LIMIT 5"
