...
# ----------------------------------------------------------------------
@register.filter
def xtd_comment_gravatar_url(email, size=40):
    try:
        user = UserProfile.objects.get(email=email)
        avatar_url = user.avatar
        url = ('/media/%s' % avatar_url)
    except:
        url = ("http://www.gravatar.com/avatar/%s?%s&d=mm" % (hashlib.md5(email.lower().encode('utf-8')).hexdigest(), urlencode({'s': str(size)})))
    return url


# ----------------------------------------------------------------------
@register.filter
def xtd_comment_gravatar(email, size=40):
